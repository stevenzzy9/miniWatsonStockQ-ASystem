...
public class TernarySearchTrie {
	...
	public static void main(String[] args) throws Exception {
		TernarySearchTrie dictionary = new TernarySearchTrie(new File("SDIC.txt"));
		dictionary.save("TrieTree");
		//benchMark(args[0]);
		//benchMark(args[1]);
	}
	...
	/**
	 *  Compares characters by alfabetical order.������ĸ��˳������
	 *
	 ...
	 *      char is less, equal or greater.//����һ���������ڶ����ַ�С�ڵ�һ���ַ�0�����ڶ����ַ���ڵ�һ���ַ�������һ���������ڶ����ַ���ڵ�һ���ַ�
	 */
	private static int compareCharsAlphabetically(int cCompare2, int cRef) {//ע��char�����ǿ���ֱ�ӵ�ת����int���͵�
	/*	int i = 0;ʧ�ܵĲ���
		while(i<10){
		System.out.println("key.charAt(charIndex):"+cCompare2+" "+"currentNode.splitchar:"+cRef);
		i++;
		}*/
		int cCompare = 0;
		if (cCompare2 >= 65) {
	...
	public TernarySearchTrie(File file, boolean compression) throws IOException {
		...
		while ((word = in.readLine()) != null) {
			...
			if (pos != -1) {
				...
				word = word.substring(0, pos).toLowerCase();
	...
	public Object get(String key) {
		TSTNode node = getNode(key.trim()/*StringUtils.toLowerCase(key.trim(), false)*/);
		if (node == null) { return null; }
		return node.data;//ֻ�Ǵ��һ��char�ڵ���dataֵ
	}
	...
	public Integer getAndIncrement(String key) {
		String key2 = key.trim();/*StringUtils.toLowerCase(key.trim(), false);*/
	 ...
	 */
	protected String getKey(TSTNode node) {//��ݽڵ�õ�����Ĵ�
	...
	public TSTNode getNode(String key) {
		/*System.out.println("kkkkkkkkkk"+rootNode);*/
	...
	protected TSTNode getNode(String key2, TSTNode startNode) {
		String key = key2.trim();/* StringUtils.toLowerCase(key2.trim(), false); */
	...
	protected TSTNode getOrCreateNode(String key)
		...
		if (rootNode == null) {//Ϊ���ݴ�Ĵ���
			rootNode = new TSTNode(key.charAt(0), null);
		}
		TSTNode currentNode = rootNode;//������Ҳ�ǿ��Կ��������Ǵ����ڵ�Ҳ�ǴӸ�ڵ㿪ʼ�ġ�!!�����currentNode�ĸ����ʵ����̫ǿ�ˣ���Ĵ������п����Ǹ��ڵ㣬�п������ӽڵ㡣
		int charIndex = 0;
		while (true) {
			int charComp =
				compareCharsAlphabetically(
					key.charAt(charIndex),
					currentNode.splitchar);
			if (charComp == 0) {//������keyֵ��Ҳ��������Ҫ�����word�����д��ھ�ֱ�ӷ��أ��жϴ���Ҳ�ǴӸ�һ��һ���ıȽϵ�
				charIndex++;
				if (charIndex == key.length()) {
					return currentNode;
				}
				if (currentNode.relatives[TSTNode.EQKID] == null) {
					currentNode.relatives[TSTNode.EQKID] =//�����ֵ��丳ֵ����һ����ַ,a[0]=c;
						new TSTNode(key.charAt(charIndex), currentNode);//�����currentNode��ʾ���ǵ�ǰ�ڵ�ĸ��ڵ�
				}
				currentNode = currentNode.relatives[TSTNode.EQKID];//b=a[0];
			} else if (charComp < 0) {
				if (currentNode.relatives[TSTNode.LOKID] == null) {//���currentNode������û��ֵ��ô�͸���ֵ
					currentNode.relatives[TSTNode.LOKID] =
						new TSTNode(key.charAt(charIndex), currentNode);
				}
				currentNode = currentNode.relatives[TSTNode.LOKID];//�����ֵ��ָ��ͼ���������ƶ���
			} else {
	...
	public void put(String key, Object value) {
		getOrCreateNode(key.trim()/*StringUtils.toLowerCase(key.trim(), false)*/).data =
			value;
	}
	...
	public void remove(String key) {
		deleteNode(getNode(key.trim()/*StringUtils.toLowerCase(key.trim(), false)*/));
	}
	 ...
	 *@return                  A <code>List</code> with the results.
	 */
	protected List sortKeys(TSTNode startNode, int numReturnValues) {//(rootNode, -1)//int numReturnValues���������Ϊ�����Ʒ��صĽڵ���-1�������ʾ������
		return sortKeysRecursion(
			startNode,
			((numReturnValues < 0) ? -1 : numReturnValues),
			new Vector());//������һ�����ݻᷢ��仯�Ľ��
		...
		int sortKeysNumReturnValues,List sortKeysResult2)
	{
		...
		if (currentNode.data != null) {//���ҵ�����Ƶ�ʱ�ʾ���Ǹ��ڵ�ʱ�ͼ��뵽���ص�List��ȥ
			sortKeysResult.add(new TSTItem(getKey(currentNode),currentNode.data));//�����sortKeysResult2��sortKeysResult�ķֽ��ߡ�
		}
    ...
    protected void outputBalanced(BufferedWriter  fp,List k,int offset, int n)
    				throws Exception////fp ��������Ҫ��ɵ�һ���µ��ļ� ret����һ�������źõ�list n�������list����Ĵ�С
	{
        int m;
        if (n < 1) {
            return;
    ...
    public void save(String sFilename) throws Exception
	{
        ...
        List ret = sortKeys(rootNode, -1);
        int n = ret.size();
        
        //out put in balance order
        outputBalanced(fp,ret,0, n);//fp ��������Ҫ��ɵ�һ���µ��ļ� ret����һ�������źõ�list n�������list����Ĵ�С
